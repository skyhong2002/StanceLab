<script lang="ts">
	...
	let customMode = $state(untrack(() => value !== '' && !SUGGESTED_MODELS.includes(value)));
	...
	const currentMeta = $derived<ModelOption | undefined>(
		MODEL_OPTIONS.find((m) => m.id === value)
	);
	...
	function openMenu() {
		...
		highlighted = Math.max(
			0,
			MODEL_OPTIONS.findIndex((m) => m.id === value)
		);
	...
	function enterCustom() {
		customMode = true;
		customDraft = value && !SUGGESTED_MODELS.includes(value) ? value : '';
			...
			>
				...
				<ul class="model-list">
					...
					{#if filtered.length === 0}
						<li class="model-empty">No matches. Try a different search, or use a custom ID.</li>
					{/if}
					<li>
						<button
							type="button"
						...
						>
							...
							<span class="model-option-body">
								...
								<span class="model-option-blurb">
									Paste any OpenRouter-compatible model identifier.
								</span>
							</span>
						</button>
					</li>
